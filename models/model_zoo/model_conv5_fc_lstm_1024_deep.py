...
def encoder_model(frames, sequence_length, initializer, scope='encoder', fc_conv_layer=False):
  ...
  for i in range(sequence_length):
    ...
    with tf.variable_scope(scope, reuse=reuse):
      ...
      hidden5, lstm_state5 = basic_conv_lstm_cell(conv5, lstm_state5, 128, initializer, filter_size=3, scope='convlstm5')
...
def decoder_model(hidden_repr, sequence_length, initializer, num_channels=3, scope='decoder', fc_conv_layer=False):
  ...
  for i in range(sequence_length):
    ...
    with tf.variable_scope(scope, reuse=reuse):
      ...
      fc_conv = slim.layers.conv2d_transpose(hidden0, 128, [4, 4], stride=1, scope='fc_conv', padding='VALID', weights_initializer=initializer)


      #LAYER 1: convLSTM1
      hidden1, lstm_state1 = basic_conv_lstm_cell(fc_conv, lstm_state1, 128, initializer, filter_size=3, scope='convlstm1')
