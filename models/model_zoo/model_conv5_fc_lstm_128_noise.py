...
def composite_model(frames, encoder_len=5, decoder_future_len=5, decoder_reconst_len=5, noise_std = 0.0, uniform_init=True, num_channels=3, fc_conv_layer=True):
  """
  Args:
    frames: 5D array of batch with videos - shape(batch_size, num_frames, frame_width, frame_higth, num_channels)
  ...
  hidden_repr = hidden_repr + tf.random_normal(shape=hidden_repr.get_shape(), mean=0.0, stddev=noise_std, dtype=tf.float32)
